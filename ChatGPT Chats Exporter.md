...
Install the bundled `dist/chatgpt-chats-exporter.user.js` in Tampermonkey or Violentmonkey. Save the script, open or refresh a ChatGPT conversation while signed in, and use the floating **Export HTML** button.
...
The internal ChatGPT endpoint and session response are undocumented web-app interfaces and may change. Authentication, rate limiting, workspace permissions, archived chats, project-specific access, or endpoint changes can produce different results. The adapter exports the active branch only. The prompt rail is user-only by design. Rich non-text content is marked rather than interpreted.
