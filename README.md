...
| ChatGPT image asset pointers on a best-effort basis with expanded response-shape and file-download handling | Exact provider token/context-window usage statistics |
...
The generated HTML follows the older thread-export archive style: a compact light 820 px layout, GitHub-like metadata, white message cards, lavender user cards, dark code blocks, and teal Copy controls. The renderer uses real paragraphs and line breaks, so it intentionally does not apply `white-space: pre-wrap` to the message content. Each export identifies itself with `chatgpt-thread-archiver 0.7.0` in metadata and the footer. When a provider exposes a model name, the header reports it; the header also reports safe local word, character, and text-block counts derived from the exported content. When ChatGPT images are present, it reports embedded and unavailable image counts.
