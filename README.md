...
Install the bundled `dist/chatgpt-chats-exporter.user.js` in Tampermonkey or Violentmonkey. Because the 0.3.0 name and namespace changed to `ChatGPT Thread Archiver` and `local.chatgpt-thread-archiver`, delete the old userscript entry manually before reinstalling this build. The 0.4.0 image resolver is included in the generated bundle. Save the script, open or refresh a ChatGPT conversation while signed in, and use the floating **Export HTML** button.
