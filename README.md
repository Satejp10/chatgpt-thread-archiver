...
| ChatGPT image asset pointers on a best-effort basis with expanded response-shape and file-download handling; include-all, exclude-all, and individual image choices | Exact provider token/context-window usage statistics |
...
| Prompt navigation rail, copy buttons, role counts, model labels, and safe local statistics | Official ZIP/conversations.json import |
| Explicit omission counts and failure diagnostics | Claude image embedding and rich-content fidelity |

Unsupported non-text blocks are represented with visible omission markers and counted in the exported HTML. Recognized image asset pointers, nested image records, execution-output images, and already-embedded image data are resolved through same-origin ChatGPT file metadata and signed content URLs, then embedded as `data:` URLs so the result remains self-contained offline. The export dialog can include all ChatGPT images, exclude all images, or let the user choose individual images before any selected-image downloads begin. Only image MIME types are accepted, each embedded image is capped at 3 MiB, and expired, inaccessible, oversized, or unsupported assets receive an explicit `[image unavailable: ...]` marker rather than being silently omitted. The exporter deliberately fails instead of generating a successful-looking partial file when the conversation response cannot be recognized. Claude exports are intentionally text-first in v0.6: unsupported thinking, tool, file, and attachment content receives a visible omission marker rather than being silently dropped.
...
Click the floating **Export HTML** button and choose whether to include the conversation URL, title, and conversation-ID metadata. For ChatGPT, choose **Include all images**, **Exclude all images**, or **Choose images after loading the conversation**. Excluded images are never downloaded and are marked clearly in the HTML. You can also turn **Show a model label on each assistant message** on or off. When the provider gives no usable model identifier, the export shows `Model: unknown / not found` rather than guessing. Disabled fields are omitted entirely, not written blank. The title is also used in the filename when enabled; otherwise the filename falls back to a timestamped `chatgpt-export-...` name.

The generated HTML follows the older thread-export archive style: a compact light 820 px layout, GitHub-like metadata, white message cards, lavender user cards, dark code blocks, and teal Copy controls. The renderer uses real paragraphs and line breaks, so it intentionally does not apply `white-space: pre-wrap` to the message content. Each export identifies itself with `chatgpt-thread-archiver 0.8.1` in metadata and the footer. When a provider exposes a model name, the header reports it; the header also reports safe local word, character, and text-block counts derived from the exported content. When enabled, each assistant message also shows its model identifier or `Model: unknown / not found`. When ChatGPT images are present, the header reports embedded, excluded, and unavailable image counts.
...
src/export-stats.mjs       Local word, character, block, role, and model statistics
