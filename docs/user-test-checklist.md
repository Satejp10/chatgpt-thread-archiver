...
| Claude branch | Export a Claude conversation with a regenerated response while a specific branch is selected | The current branch is exported in order by default; alternate branches are not duplicated unless the all-branch option is selected |
...
| Regeneration | Export a chat with a regenerated assistant response | Only the active current branch is exported by default; alternate branches are not duplicated |
...
| Branch default and persistence | Export once with all branches, then reopen the dialog and export another conversation without changing the setting | The selected branch mode follows the documented preference; current-branch mode remains available and does not include alternate paths when selected |
