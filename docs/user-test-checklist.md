...
| Provenance | Inspect the HTML head and footer | Generator metadata and footer identify `chatgpt-thread-archiver 0.13.2` |
...
| Generated-image model discovery | In ChatGPT, create one fresh generated image and export it with images included; inspect the image caption and any provider-safe metadata shown by the exporter | If an exact image-specific model field is exposed, the caption shows that exact identifier; otherwise it says `Image model: unknown / not found`; no image bytes, signed URLs, cookies, response bodies, or full private IDs are written to the export |
| Claude branch | Export a Claude conversation with a regenerated response while a specific branch is selected | Every branch is exported by default, the selected branch renders first, and the local `‹`/`›` controls reach the others |
| Switcher placement | Export a conversation with an edited prompt whose reply was also regenerated | Each `‹ 1/2 ›` sits directly above the block it switches — one above the prompt, one above the reply — not stacked together at the end |
...
| New chat without reload | On the ChatGPT or Claude home page, start a new chat and wait for the reply | **Export HTML** appears without refreshing |
| Excluded ChatGPT sections | Open `chatgpt.com/codex/`, `/scheduled`, `/library` | No **Export HTML** button |
...
| Regeneration | Export a chat with a regenerated assistant response | Every version is exported by default and reachable through the fork controls; choosing **Export current branch only** limits the file to the active branch |
| ChatGPT Projects route | Open a specific conversation inside a ChatGPT Project at `/g/...`, then refresh and export | The button appears, the export retrieves that same conversation, and the HTML title/message content match the open Project conversation; if the route does not expose a conversation ID, the exporter fails clearly rather than exporting an unrelated chat |
| All branches | Open a conversation with an edited prompt or regenerated response, choose **Include edited and regenerated branches**, and export | The file labels the available branches, starts on the active branch, lets the local `‹`/`›` controls switch branches, and shows no provider edit/regenerate actions |
| Branch default and persistence | Export once on a fresh install, then switch to **Export current branch only**, reopen the dialog, and export another conversation | A fresh install starts on **Include edited and regenerated branches**; a deliberate switch to current-branch-only persists and is not reset back |
| Branch no-JavaScript fallback | Disable JavaScript before opening an all-branch export | All labelled branch paths remain readable in document order, with no unusable hidden-only transcript |
| Image embedding | Export a conversation containing an uploaded or generated image that is still available with **Include all images** selected | The HTML contains a visible image, the header reports it as embedded, and the image remains visible after opening the file offline |
...
| Exclude all images | Choose **Exclude all images** for a conversation containing available images | No image asset requests occur after the conversation loads; the HTML contains exclusion markers and reports the excluded count |
| Choose individual images | Choose **Choose images after loading the conversation**, uncheck one or more images, and export | Only checked images are downloaded/embedded; unchecked images are marked excluded and the header counts them correctly |
| Image mode persistence | Export once with **Exclude all images**, then open the options again; later switch to **Choose images** | The broad mode preference is remembered, but individual image selections are not reused for a different export |
...
| Large image export | Export a conversation whose images total well over 12 MiB, including at least one image over 3 MiB, with every image selected | Every selected image embeds; no image is dropped for a size or count budget and the header reports zero budget-limited images |
| Image selection controls | In the image chooser, toggle **Select all** on and off, then uncheck a single image | Select all clears and restores the whole list; with a partial selection it shows the mixed/indeterminate state, and the summary line tracks the selected count and total size |
| Throttled image download | Export a large image set and observe behaviour if ChatGPT rate-limits part-way through | A throttled image is retried once rather than dropped; a repeated refusal shows an explicit unavailable marker rather than failing the whole export |
...
| Withheld branches are not an error | Choose **Export current branch only** on a conversation that has edits or regenerations | The header states in plain type how many messages sit on other branches and which setting keeps them; it does not show a yellow `dropped nodes` warning |
